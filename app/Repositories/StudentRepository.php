...
class StudentRepository
{
    ...
    {
        $studentsWhoPaid = $this->model->with('schoolMajor', 'schoolClass')->whereHas('deposits', function ($query) use ($startDate, $endDate) {
            return $query->whereBetween('date_paid', [$startDate, $endDate]);
        })->get();

        $studentsWhoDidNotPay = $this->model->with('schoolMajor', 'schoolClass')->whereDoesntHave('deposits', function ($query) use ($startDate, $endDate) {
            return $query->whereBetween('date_paid', [$startDate, $endDate]);
        })->get();
