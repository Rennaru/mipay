<div>
  ...
  <div class="row">
    <div class="col-12">
      <div class="card">
        <div class="card-body">
          <h5 class="card-title">Daftar Transaksi Pembayaran Minggu Ini</h5>
          ...
          <div class="table-responsive">
            <table class="table table-bordered table-hover">
              ...
              <tbody>
                ...
                @php
                $startIndex = ($deposits->currentPage() - 1) * $deposits->perPage() + 1;
                @endphp

                @forelse ($deposits as $index => $deposit)
                <tr wire:key="{{ $deposit->id }}">
                  <th scope="row">{{ $startIndex + $index }}</th>
                  <td class="text-uppercase fw-bold">{{ $deposit->student->name }}</td>
                  <td>{{ local_amount_format($deposit->amount) }}</td>
                  ...
                  <td>
                    <div class="btn-group gap-1" role="group">
                      <button wire:loading.attr="disabled"
                        wire:click="$dispatch('deposit-edit', {deposit: {{ $deposit->id }}})"
                        type="button" class="btn btn-sm btn-success rounded" data-bs-toggle="modal"
                        data-bs-target="#editModal">
                        <i class="bi bi-pencil-square"></i>
                      </button>
                      <button wire:loading.attr="disabled"
                        wire:click="$dispatch('deposit-delete', {deposit: {{ $deposit->id }}})"
            ...
            {{ $deposits->links(data: ['scrollTo' => false]) }}
          </div>
        </div>
      </div>
    </div>

    <livewire:deposits.create-deposit :$students />
    <livewire:deposits.edit-deposit :$students />
    <livewire:deposits.delete-deposit />
  </div>
